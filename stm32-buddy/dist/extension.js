...
function activate(context) {
  ...
  const explainFunctionCommand = vscode.commands.registerCommand("commentgen.explainFunction", async () => {
    ...
    if (!editor) {
      vscode.window.showErrorMessage("Aucun \xE9diteur actif.");
      return;
    }
    const selection = editor.selection;
    const selectedText = editor.document.getText(selection);
    if (!selectedText.trim()) {
      vscode.window.showErrorMessage("Aucune fonction s\xE9lectionn\xE9e.");
      return;
    }
    try {
      const response = await axios_default.post("http://127.0.0.1:8000/generate-comment", {
        code: selectedText
      });
      const comment = response.data.commented_code;
      const commentedLines = comment.split("\n").map((line) => `// ${line}`).join("\n");
      await editor.edit((editBuilder) => {
        editBuilder.insert(selection.start, `${commentedLines}
`);
      });
      vscode.window.showInformationMessage("Commentaire ajout\xE9 avec succ\xE8s.");
    } catch (error) {
  ...
  let disposable = vscode.commands.registerCommand("bugDetector.detectBug", async () => {
    ...
    try {
      ...
      if (data.prediction === "buggy") {
        vscode.window.showWarningMessage("\u{1F6A8} The selected code is likely buggy.");
      } else {
  ...
  const detectErrorType = vscode.commands.registerCommand("bugDetector.errorType", async () => {
    ...
    try {
      const response = await axios_default.post("http://localhost:8000/predict-error-type", {
        code: selectedText
      });
      const errorType = response.data.error_type || "Unknown";
      vscode.window.showInformationMessage(`\u{1F9E0} Type d'erreur d\xE9tect\xE9e : ${errorType}`);
    } catch (error) {
      const message = error.response?.data?.detail || error.message || "Erreur inconnue";
      vscode.window.showErrorMessage(`Erreur API : ${message}`);
    }
  ...
  const analyzeLines = vscode.commands.registerCommand("bugDetector.lineByLine", async () => {
    ...
    try {
      const response = await axios_default.post("http://localhost:8000/predict-lines-error-type", {
        code: selectedText
      });
      const result = response.data.result || [];
      if (result.length === 0) {
        vscode.window.showInformationMessage("\u2705 Aucun probl\xE8me d\xE9tect\xE9 ligne par ligne.");
        return;
      }
      const output = vscode.window.createOutputChannel("STM32 Line Analyzer");
      output.clear();
      output.appendLine("\u{1F50D} R\xE9sultat de l'analyse ligne par ligne :\n");
      result.forEach((item, index) => {
        output.appendLine(`Ligne ${index + 1}: ${item.line}`);
        output.appendLine(`   \u{1F6D1} Erreur: ${item.error_type}
`);
      });
      output.show();
    } catch (error) {
  ...
  const inlineProvider = {
    provideInlineCompletionItems: async (document2, position, context2, token) => {
      const linePrefix = document2.lineAt(position).text.substring(0, position.character);
      try {
        const response = await axios_default.post("http://127.0.0.1:8000/stm32-autocomplete", {
          prefix: linePrefix
        });
  ...
  const generateCodeDisposable = vscode.commands.registerCommand("t5-code-generator.generateCode", async () => {
    ...
    if (prompt) {
      try {
        ...
        const response = await axios_default.post("http://127.0.0.1:5000/generate", { prompt });
  ...
  const generateTestCodeDisposable = vscode.commands.registerCommand("t5-code-generator.generateTestCode", async () => {
    ...
    if (functionCode) {
      try {
        ...
        const response = await axios_default.post("http://127.0.0.1:7000/generate-test", {
          function_code: functionCode
        });
        const generatedTestCode = response.data.generated_test;
